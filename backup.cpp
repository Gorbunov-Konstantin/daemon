...
void performBackup(const std::string& sourceDir, const std::string& backupDir) {
    try {
        ...
        if (!fs::exists(sourcePath) || !fs::is_directory(sourcePath)) {
            syslog(LOG_ERR, "Source directory does not exist or is not a directory");
            return;
        }

        if (!fs::exists(backupPath)) {
            fs::create_directories(backupPath);
            syslog(LOG_INFO, "Created a new directory for backups");
        }
        ...
        for (const auto& entry : fs::directory_iterator(sourcePath)) {
            ...
            syslog(LOG_INFO, "Copied file: %s", entry.path().filename().c_str());
        }
...
int main() {
    ...
    syslog(LOG_INFO, "Backup daemon is running");
